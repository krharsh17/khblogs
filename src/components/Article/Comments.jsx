...
const CommentBoxInput = styled.textarea`
  ...
  &:focus {
    outline: none;
    border: 1.5px solid rgba(39, 41, 45, 1);
  }
          ...
          <CommentBoxFooter>
            ...
            <CommentBoxPost
              ...
              onClick={() => {
                if (database !== undefined) {
                  database
                    .ref("articles/" + props.id + "/comments/")
                    .push({
                      name: name,
                      email: email,
                      comment: comment,
...
const Comments = props => {
        ...
        .on("value", snapshot => {
          ...
          if (snapshot.val() !== null) {
            Object.values(snapshot.val()).forEach(elem => {
              const temp = (
                <Comment
                  key={elem.email + Date.now() + Math.random() * 20}
  ...
  if (loading === "true") {
    return (
      ...
      >
        <ClipLoader size={50} color={"#121212"} loading={true} />
      </div>
    )
