...
const HomeBody = () => {
  ...
  useEffect(() => {
    initDB()
    if (
      window.innerWidth > parseInt(device.mobileL.split("px")[0].split(" ")[1])
    )
      mainArticlesCount = 6
    let articles = []
    ...
    if (articles.length > 0) setState({ articles: articles })
  }, [])
  ...
  return (
    <HomeBodyContainer>
        ...
        {state.articles.slice(0, mainArticlesCount).map((article, index) => {
          return (
            <PostContainer key={index}>
              ...
              <PostRead href={article !== undefined ? article.path : ""}>
                Read Article
              </PostRead>
            </PostContainer>
          )
      ...
      <PostsContainer>
        {state.articles
          .slice(mainArticlesCount, mainArticlesCount + 3)
          .map((article, index) => {
            return (
              <PostContainer key={index}>
                <PostImage src={article !== undefined ? article.imgUrl : ""} />
